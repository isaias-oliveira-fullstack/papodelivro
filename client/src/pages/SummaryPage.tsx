  ...
  useEffect(() => {
    const fetchMissingBookData = async () => {
      ...
      try {
        ...
        console.error(
          "Falha ao buscar dados do livro para edição de resumo:",
          fetchError,
        );
      }
    };
            ...
            {prefilledBook && (
              <div className="mt-8 rounded-4xl border border-slate-200 bg-slate-50 p-6 dark:border-slate-700 dark:bg-slate-950">
                {prefilledBook.cover_url && (
                  <img
                    src={getImageUrl(prefilledBook)}
                    alt={`Capa de ${prefilledBook.title}`}
                    className="mb-4 h-48 w-full rounded-3xl object-cover"
                  />
                )}
                <p className="text-slate-700 dark:text-slate-300">
                  Livro: <strong>{prefilledBook.title}</strong> por{" "}
                  {prefilledBook.author}
                </p>
              </div>
            )}
          </div>
                ...
                <div className="rounded-4xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-950">
                  ...
                  <label className="mt-6 block rounded-4xl border border-slate-300 bg-white p-4 text-slate-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">
                    <span className="text-sm font-medium">
                      Alterar imagem de capa (opcional)
                    </span>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-slate-500 dark:text-slate-400">
                  Acompanhe o status dos seus resumos enviados.
                </p>
              </div>
              {loadingMySummaries ? (
                <p className="text-slate-600 dark:text-slate-400">
                      ...
                      >
                        {summary.cover_url && (
                          <img
                            ...
                            className="h-16 w-12 rounded-2xl object-cover"
                          />
                        )}
                ...
                <p className="text-slate-600 dark:text-slate-400">
                  Você ainda não enviou nenhum resumo.
                </p>
              )}
            </div>
          </aside>
