...
const BookPage = () => {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) => {
        console.error(
          "Erro ao buscar livros da API, usando dados locais:",
          err,
        );
    ...
    <section className="pb-12 space-y-6">
      <div className="bg-gray-200 py-6 transition dark:bg-slate-900/95 sm:p-8">
        <div className="max-w-auto md:max-w-7xl px-4 mx-auto">
          <h2 className="text-3xl font-semibold mb-0">Livros</h2>
          <p className="mt-2 text-sm">Descubra novos livros e resumos</p>
        </div>
      </div>
      <div className="max-w-auto md:max-w-7xl px-4 mx-auto">
        <div className="rounded-lg border border-slate-200 bg-white/90 p-6 shadow-sm transition dark:border-slate-700 dark:bg-slate-900/95 sm:p-8 mb-2">
          <div className="flex flex-col gap-6 items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                Filtre livros por temas e descubra novos favoritos.
              </p>
            </div>
        ...
        <div className="mb-6">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-300 bg-white px-4 py-4 text-slate-900 outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:focus:border-slate-500 dark:focus:ring-slate-800"
          />
        </div>
        ...
        {filteredBooks.length === 0 ? (
          ...
          <>
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
              {filteredBooks.slice(0, visibleCount).map((livro) => (
            ...
            {visibleCount < filteredBooks.length && (
              <div className="mt-8 flex justify-center">
                <button
                  ...
                  className="rounded-full bg-slate-950 px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-200"
                >
